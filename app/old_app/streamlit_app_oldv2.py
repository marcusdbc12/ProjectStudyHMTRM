...
st.title('j-Wave + Keras Workflow for Silicon and Pd/Si Defect Imaging')
...
with left:
    ...
    structure_mode = st.selectbox('Structure mode', ['Pd/Si', 'Pure Silicon'], index=0, help='Choose whether the generated sample is a Pd(50 nm)/Si(300 µm) layered structure or pure silicon.')
    ...
    if structure_mode == 'Pd/Si':
        ...
        pd_thickness_nm = 0.0
        pd_speed = 3070.0
        st.caption('Pure Silicon mode selected: Pd thickness is forced to 0 nm and Pd-related defect classes are disabled in the generator.')
    ...
    if st.button('Generate synthetic data'):
        cmd = [
            ...
            '--structure-mode', 'pd_si' if structure_mode == 'Pd/Si' else 'pure_si',
    ...
    if st.button('Split data'):
        cmd = [sys.executable, str(SCRIPTS / 'split_dataset.py'), '--project-root', str(ROOT), '--train-ratio', str(train_ratio), '--validation-ratio', str(val_ratio), '--test-ratio', str(test_ratio), '--copy-test-files']
        log_box = st.empty(); lines = []
        try:
