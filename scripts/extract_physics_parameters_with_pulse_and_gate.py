...
def build_physics_summary(args_df: pd.DataFrame) -> pd.DataFrame:
    ...
    rows = [
        ...
        {'parameter': 'frequency', 'value': frequency, 'units': 'Hz', 'meaning': 'Source center frequency'},
...
def build_pulse_summary(args_df: pd.DataFrame) -> pd.DataFrame:
    ...
    rows = [
        {'parameter': 'frequency', 'value': frequency, 'units': 'Hz', 'why_used': 'Sets the oscillation rate of the pulsed-laser source waveform.', 'do_we_use_it': 'yes'},
        {'parameter': 'period', 'value': period_s, 'units': 's', 'why_used': 'One acoustic cycle duration = 1/frequency.', 'do_we_use_it': 'derived from frequency'},
        {'parameter': 'source_center_fraction', 'value': source_center_frac, 'units': 'fraction of t_end', 'why_used': 'Places the pulse away from t=0 so the waveform has quiet time before emission.', 'do_we_use_it': 'yes, implicit script constant'},
        {'parameter': 'source_center_time', 'value': source_center_time, 'units': 's', 'why_used': 'Approximate center of the Gaussian-windowed pulse used by the source generator.', 'do_we_use_it': 'yes'},
        {'parameter': 'source_center_time_ns', 'value': source_center_time * 1e9, 'units': 'ns', 'why_used': 'Same pulse-center time expressed in nanoseconds for interpretation.', 'do_we_use_it': 'derived view'},
        {'parameter': 'window_rule_fraction', 'value': source_width_rule1, 'units': 's', 'why_used': 'One candidate width: 10% of the full simulation time window.', 'do_we_use_it': 'yes, in max(...) rule'},
        {'parameter': 'window_rule_cycles', 'value': source_width_rule2, 'units': 's', 'why_used': 'Second candidate width: approximately three periods of the source frequency.', 'do_we_use_it': 'yes, in max(...) rule'},
        {'parameter': 'window_width_used_in_source', 'value': source_width_used, 'units': 's', 'why_used': 'Actual width passed to the Gaussian source window = max(0.10*t_end, 3/f).', 'do_we_use_it': 'yes'},
        {'parameter': 'window_width_used_in_source_ns', 'value': source_width_used * 1e9, 'units': 'ns', 'why_used': 'Same source-window width in nanoseconds.', 'do_we_use_it': 'derived view'},
        {'parameter': 'gate_margin_time', 'value': margin_time, 'units': 's', 'why_used': 'Extra safety delay added after estimated direct arrival before gating.', 'do_we_use_it': 'yes'},
        {'parameter': 'gate_margin_time_ns', 'value': margin_time * 1e9, 'units': 'ns', 'why_used': 'Same gate margin in nanoseconds.', 'do_we_use_it': 'derived view'},
    ]
    return pd.DataFrame(rows)
...
def build_probability_summary(args_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for name in prob_args:
        ...
        rows.append({'class_argument': name, 'default_weight': val, 'normalized_probability': val / total if total > 0 else None})
    return pd.DataFrame(rows)
...
def build_markdown(args_df: pd.DataFrame, physics_df: pd.DataFrame, pulse_df: pd.DataFrame, probs_df: pd.DataFrame, gate_df: pd.DataFrame) -> str:
    return f'''# Physics, pulse, and gate parameter extraction report
...
and computes the gate window per receiver using the current fixed formula:
...
## Why include pulse parameters
Because the source waveform is not emitted at `t = 0`. The waveform is centered later
in the acquisition window and has a finite width. Those values matter when explaining:
- why the traces are faint at early times,
- why the old gate bug removed almost all signal,
- and how the corrected gate is constructed.

## What to look at
- `physics_parameter_summary.csv` contains the full simulation time per sample
- `pulse_parameter_summary.csv` contains pulsed-laser timing choices and why they are used
...
- pulse-related quantities: {len(pulse_df)}
...
def main() -> None:
    ...
    args_csv = out_dir / 'cli_argument_defaults.csv'
    physics_csv = out_dir / 'physics_parameter_summary.csv'
    pulse_csv = out_dir / 'pulse_parameter_summary.csv'
    probs_csv = out_dir / 'class_probability_summary.csv'
    sample_csv = out_dir / 'dataset_sample_summary.csv'
    gate_csv = out_dir / 'gate_window_summary.csv'
    md_path = out_dir / 'physics_parameter_report.md'

    args_df.to_csv(args_csv, index=False)
    ...
    md_path.write_text(build_markdown(args_df, physics_df, pulse_df, probs_df, gate_df), encoding='utf-8')

    print(f'Saved: {args_csv}')
    print(f'Saved: {physics_csv}')
    print(f'Saved: {pulse_csv}')
    print(f'Saved: {probs_csv}')
