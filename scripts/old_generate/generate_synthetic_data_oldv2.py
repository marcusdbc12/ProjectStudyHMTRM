...
def top_surface_line_positions(nx: int, count: int, y_index: int = 1, margin_x: int | None = None):
    """
    Return evenly spaced positions along the top surface line.
    count measurement/source positions on row y_index.
    """
    if count < 1:
...
def normalize_recordings_array(recordings: np.ndarray, n_sensors: int) -> np.ndarray:
    """
    Normalize j-Wave sensor output to shape (time, sensors).
    """
...
def normalize_field_array(field: np.ndarray) -> np.ndarray:
    """
    Normalize j-Wave field output to shape (time, ny, nx).
    """
...
def plot_sample(
    ...
    axes[1, 1].set_title('TRM max-amplitude map')
    ...
    plt.plot(traces[:, 0, 0])
    plt.title(f'{sample_id} forward src0/rx0')
    plt.tight_layout()
...
def main() -> None:
    ...
    # Realistic local-window geometry for a virtual TRM:
    # one laser source near the top surface + a line of measurement positions
    # along the same accessible top surface.
    args.sources = 1
    source_y, source_x = top_surface_line_positions(nx, 1, y_index=1)
    ...
    for i in range(args.samples):
        ...
        # Forward step: one physical source, multiple measurement positions
        src_forward = Sources(
            positions=(np.array([source_y[0]]), np.array([source_x[0]])),
            signals=signal[None, :],
            dt=time_axis.dt,
            domain=domain,
        )
        ...
        forward_traces = normalize_recordings_array(recordings, args.sensors)
        traces_arr = forward_traces[:, None, :]  # keep (time, sources, sensors) convention with one source

        # Numerical TRM step:
        # reverse each measured trace in time and re-emit from the same positions
        traces_tr = forward_traces[::-1, :]

        src_trm = Sources(
            positions=(sensor_y, sensor_x),
            signals=jnp.asarray(traces_tr.T),
            dt=time_axis.dt,
            domain=domain,
        )
        ...
        # Keep the legacy filename so downstream scripts still work.
    ...
    manifest = {
        ...
        'notes': (
            'Pd/Si interface-sensitive synthetic data with a one-source virtual TRM setup. '
