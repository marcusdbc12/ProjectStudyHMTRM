...
"""
Generate synthetic j-Wave data with a selectable pulsed-laser source position.

This version is based on the trace-gated numerical TRM workflow:
1) forward simulation from one source
2) record traces on a top-surface receiver line
3) gate the forward traces using estimated direct-arrival time + margin
4) reverse the gated traces in time
5) backpropagate from the same receiver positions
6) save TRM max map and TRM energy map

New source-position controls:
- --source-position {left,center,right,custom}
- --source-x-px
- --source-y-px
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Generate synthetic j-Wave data with selectable source position")
    ...
    p.add_argument("--source-x-px", type=int, default=None,
                   help="Exact lateral source position in pixels when using custom mode.")
...
def ellipse_mask(shape: tuple[int, int], cx: int, cy: int, a: int, b: int, theta: float) -> np.ndarray:
    ...
    a = max(1, a)
    b = max(1, b)
    return (((xp ** 2) / (a ** 2) + (yp ** 2) / (b ** 2)) <= 1.0).astype(np.float32)
...
def sample_defect(
    ...
    if defect_type == "no_defect":
        return defect_type, np.zeros((ny, nx), dtype=np.float32), nx // 2, ny // 2, 0, 0
...
def normalize_recordings(recordings, n_sensors: int) -> np.ndarray:
    ...
    if arr.ndim == 1:
        arr = arr[:, None]
    elif arr.ndim == 2 and arr.shape[0] == n_sensors and arr.shape[1] != n_sensors:
        arr = arr.T
    elif arr.ndim > 2:
...
def plot_sample(sample_id: str, output_dir: Path, sound_speed: np.ndarray, target: np.ndarray,
                traces: np.ndarray, trm_max: np.ndarray) -> None:
...
def choose_source_position(args: argparse.Namespace, nx: int, ny: int) -> tuple[np.ndarray, np.ndarray]:
    margin_x = max(4, nx // 10)

    if args.source_position == "left":
        sx = margin_x
    elif args.source_position == "right":
        sx = nx - 1 - margin_x
    elif args.source_position == "custom":
        sx = nx // 2 if args.source_x_px is None else int(np.clip(args.source_x_px, 0, nx - 1))
    else:
        sx = nx // 2

    sy = int(np.clip(args.source_y_px, 0, ny - 1))
...
def main() -> None:
    ...
    source_x, source_y = choose_source_position(args, nx, ny)
    logging.info("Using source position mode=%s, source=(%d,%d)", args.source_position, int(source_x[0]), int(source_y[0]))
    ...
    for i in range(args.samples):
        ...
        defect_type, target, cx, cy, w, h = sample_defect(
            rng, nx, ny, args.min_defect_frac, args.max_defect_frac, args.structure_mode, defect_type
        )
        ...
        recordings = simulate_wave_propagation(medium, time_axis, sources=src_forward, sensors=sensors)
        forward_traces = normalize_recordings(recordings, args.sensors)
        traces_arr = forward_traces[:, None, :]

        gated_traces = forward_traces.copy()
        ...
        for r in range(args.sensors):
            ...
            gate_time = t_direct + margin_time
    ...
    manifest = {
        ...
        "si_speed_m_per_s": args.si_speed,
        "notes": (
            "One-source trace-gated numerical TRM dataset with selectable source position. "
