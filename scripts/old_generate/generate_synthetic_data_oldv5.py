...
def parse_args() -> argparse.Namespace:
    ...
    # Kept for compatibility with the UI; the current workflow uses one physical pulsed-laser source.
...
def main() -> None:
    ...
    # One physical pulsed-laser source on the top surface + top-surface sensor line.
    args.sources = 1
    ...
    for i in range(args.samples):
        ...
        # Time gate based on source pulse timing + direct travel time.
        gated_traces = rec_np.copy()
        dt = float(time_axis.dt)
        margin_time = 10e-9
        source_center_time = args.t_end * 0.28
        source_width = max(args.t_end * 0.10, 3.0 / args.frequency)
        ...
        for r in range(args.sensors):
            ...
            gate_time = source_center_time + t_direct + source_width + margin_time
        ...
        # Mask the near-surface injection zone for the saved/displayed refocus image.
        # surface_mask_depth = int(sensor_y[0]) + max(5, ny // 10)
    ...
    manifest = {
        ...
        'notes': (
            ...
            'refocus.csv stores the masked TRM max map for compatibility.'
        ),
    }
