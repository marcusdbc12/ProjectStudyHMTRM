...
Variant 3: healthy-reference residual TRM.
...
3) compute a matching healthy-reference forward simulation
4) residual traces = defect traces - healthy traces
5) reverse residual traces in time
6) backpropagate from the same receiver positions
7) save TRM max map and TRM energy map
...
- Also saves *_healthy_traces.csv and *_residual_traces.csv.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Generate synthetic j-Wave data with residual numerical TRM")
...
def main() -> None:
    ...
    for i in range(args.samples):
        ...
        # Healthy-reference residual TRM
        healthy_sound_speed, healthy_density, healthy_attenuation_map = make_maps(
            nx=nx,
            ny=ny,
            defect_mask=np.zeros_like(target),
        ...
        healthy_medium = Medium(
            domain=domain,
            sound_speed=jnp.asarray(healthy_sound_speed),
            density=jnp.asarray(healthy_density),
            attenuation=jnp.asarray(healthy_attenuation_map),
        )

        healthy_recordings = simulate_wave_propagation(
            healthy_medium,
            time_axis,
            sources=src_forward,
            sensors=sensors,
        )
        healthy_traces = normalize_recordings(healthy_recordings, args.sensors)

        residual_traces = forward_traces - healthy_traces
        logging.info(
            "Residual traces stats: min=%g max=%g mean=%g",
            np.min(residual_traces), np.max(residual_traces), np.mean(residual_traces)
        )

        reversed_traces = residual_traces[::-1, :]
        ...
        pd.DataFrame(healthy_traces, columns=[f"rx_{r:02d}" for r in range(args.sensors)]).to_csv(
            input_dir / f"{sample_id}_healthy_traces.csv", index=False
        )
        pd.DataFrame(residual_traces, columns=[f"rx_{r:02d}" for r in range(args.sensors)]).to_csv(
            input_dir / f"{sample_id}_residual_traces.csv", index=False
        )
    ...
    manifest = {
        ...
        "si_speed_m_per_s": args.si_speed,
        "notes": (
            "One-source residual numerical TRM dataset. "
