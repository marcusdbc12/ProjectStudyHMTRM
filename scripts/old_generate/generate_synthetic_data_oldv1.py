...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        '--structure-mode',
        type=str,
        choices=['pd_si', 'pure_si'],
        default='pd_si',
        help='Material structure mode: Pd/Si bilayer or pure silicon.'
    )
...
def validate_probabilities(args: argparse.Namespace) -> tuple[list[str], np.ndarray]:
    if args.structure_mode == 'pure_si':
        classes = ['no_defect', 'crack', 'void']
        probs = np.array(
            [
                args.p_no_defect,
                args.p_crack,
...
def make_maps(
    ...
    if structure_mode == 'pure_si':
        # In pure silicon mode, only no_defect/crack/void should be present.
        # Keep this guard for robustness if metadata or probabilities are edited externally.
        if defect_type not in {'crack', 'void'}:
            return sound_speed, density, attenuation_map
...
def main() -> None:
    ...
    classes, class_probs = validate_probabilities(args)
    ...
    source_x = np.linspace(margin_x, nx - 1 - margin_x, args.sources, dtype=int)
    sensor_x = np.linspace(margin_x, nx - 1 - margin_x, args.sensors, dtype=int)

    source_y = np.full(args.sources, 1, dtype=int)
    ...
    manifest = {
        ...
        'pd_speed_m_per_s': args.pd_speed,
        'si_speed_m_per_s': args.si_speed,
        'class_probabilities': {name: float(prob) for name, prob in zip(classes, class_probs)},
        'notes': (
            'Synthetic layered-wave data. '
