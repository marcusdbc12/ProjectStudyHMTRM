...
def normalize_field_array(field_like) -> np.ndarray:
    """
    Convert j-Wave field outputs into a NumPy array.

    Accepts:
      - j-Wave/jaxdf field objects with `.params`
      - plain NumPy/JAX arrays

    Returns either:
      - (ny, nx) if only one final field is available
      - (nt, ny, nx) if a time history is available
    """
    obj = field_like
...
def main() -> None:
    ...
    # One physical source near top center
    ...
    for i in range(args.samples):
        ...
        forward_traces = normalize_recordings(recordings, args.sensors)  # (Nt, Nr)
        traces_arr = forward_traces[:, None, :]  # keep legacy shape (Nt, Ns=1, Nr)

        # ---------- Numerical TRM ----------
        reversed_traces = forward_traces[::-1, :]  # (Nt, Nr)

        # src_trm = Sources(
        #     positions=(sensor_y, sensor_x),
        #     signals=reversed_traces.T,  # expected (Nsources, Nt)
        #     dt=time_axis.dt,
        #     domain=domain,
        # )
        src_trm = Sources(
        ...
        # Keep legacy name for compatibility with other scripts
