...
#     # Suggested by the j-Wave FWI tutorial style
#     source_mag = 1.3e-5
#     source_mag = source_mag / time_axis.dt

#     s0 = source_mag * jnp.sin(2 * jnp.pi * frequency * t)

#     center = float(t.max()) * 0.28
#     width = max(float(t.max()) * 0.10, 3.0 / frequency)

#     s0 = apply_ramp(s0, time_axis.dt, frequency)
#     s0 = gaussian_window(s0, t, center, width)
#     return s0

def make_signal(time_axis, frequency=None):
    t = time_axis.to_array()
...
# def make_signal(time_axis, frequency=None):
#     """
#     Perfect 5 ns rectangular pulse.

#     Mathematical form:
#         s(t) = (A0 / dt) * [H(t - t0) - H(t - (t0 + tau))]
...
#     t0 = float(t.max()) * 0.28   # pulse start time
#     tau = 5.0e-9                 # 5 ns pulse duration

#     s0 = source_mag * (
#         ((t >= t0) & (t < t0 + tau)).astype(t.dtype)
#     )
#     return s0
...
#                 traces: np.ndarray, trm_max: np.ndarray, trm_max_log: np.ndarray,
#                 trm_max_row_norm: np.ndarray, trm_energy_log: np.ndarray) -> None:
...
#     axes[0, 2].imshow(traces[:, 0, :].T, aspect='auto', cmap='RdBu_r')
#     axes[0, 2].set_title('Trace stack (src 0)')

#     axes[0, 3].imshow(trm_max, cmap='inferno', aspect='auto')
#     axes[0, 3].set_title('TRM max raw')

#     axes[1, 0].imshow(trm_max_log, cmap='inferno', aspect='auto')
#     axes[1, 0].set_title('TRM max log10')

#     axes[1, 1].imshow(trm_max_row_norm, cmap='RdBu_r', aspect='auto')
#     axes[1, 1].set_title('TRM max row-normalized')

#     axes[1, 2].imshow(trm_energy_log, cmap='inferno', aspect='auto')
#     axes[1, 2].set_title('TRM energy log10')

#     combo = np.stack([
#         (trm_max_row_norm - np.min(trm_max_row_norm)) / max(np.max(trm_max_row_norm) - np.min(trm_max_row_norm), 1e-12),
#         (trm_max_log - np.min(trm_max_log)) / max(np.max(trm_max_log) - np.min(trm_max_log), 1e-12),
#         (trm_energy_log - np.min(trm_energy_log)) / max(np.max(trm_energy_log) - np.min(trm_energy_log), 1e-12),
#     ], axis=-1)
#     axes[1, 3].imshow(combo, aspect='auto')
